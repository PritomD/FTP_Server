<!DOCTYPE html>
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
<head>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<title>FTP-SERVER</title>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta name="description" content="Free HTML5 Template by FREEHTML5.CO" />
	<meta name="keywords" content="free html5, free template, free bootstrap, html5, css3, mobile first, responsive" />
	<meta name="author" content="FREEHTML5.CO" />
	
	<!-- Facebook and Twitter integration -->
	<meta property="og:title" content=""/>
	<meta property="og:image" content=""/>
	<meta property="og:url" content=""/>
	<meta property="og:site_name" content=""/>
	<meta property="og:description" content=""/>
	<meta name="twitter:title" content="" />
	<meta name="twitter:image" content="" />
	<meta name="twitter:url" content="" />
	<meta name="twitter:card" content="" />
	<!-- Place favicon.ico and apple-touch-icon.png in the root directory -->
	<link rel="shortcut icon" href="favicon.ico">
	<link href='https://fonts.googleapis.com/css?family=Roboto:400,100,300,700,900' rel='stylesheet' type='text/css'>
	<link href="https://fonts.googleapis.com/css?family=Roboto+Slab:300,400,700" rel="stylesheet">
	
	<!-- Animate.css -->
	<link rel="stylesheet" href="{{asset('template2')}}/css/animate.css">
	<!-- Icomoon Icon Fonts-->
	<link rel="stylesheet" href="{{asset('template2')}}/css/icomoon.css">
	<!-- Bootstrap  -->
	<link rel="stylesheet" href="{{asset('template2')}}/css/bootstrap.css">
	<!-- Superfish -->
	<link rel="stylesheet" href="{{asset('template2')}}/css/superfish.css">
	<!-- Flexslider  -->
	<link rel="stylesheet" href="{{asset('template2')}}/css/flexslider.css">
	<link rel="stylesheet" href="{{asset('template2')}}/css/style.css">
	<!-- Modernizr JS -->
	<script src="{{asset('template2')}}/js/modernizr-2.6.2.min.js"></script>
	<!-- FOR IE9 below -->
	<!--[if lt IE 9]>
	<script src="js/respond.min.js"></script>
	<![endif]-->
</head>
<body>
	<div id="home">
		<div id="fh5co-wrapper">
			<div id="fh5co-page">
				<div id="fh5co-header">
					<header id="fh5co-header-section">
						<div class="container">
							<div class="nav-header">
								<a href="#" class="js-fh5co-nav-toggle fh5co-nav-toggle"><i></i></a>
								<h1 id="fh5co-logo"><a><i class="icon-home2"></i>FTP-SERVER	</a></h1>
								<!-- START #fh5co-menu-wrap -->
								<nav id="fh5co-menu-wrap" role="navigation">
									<ul class="sf-menu" id="fh5co-primary-menu">
										<li>
											<a class="active" href="/">Home</a>
										</li>
										
										<li>
											<a class="fh5co-sub-ddown">Categories</a>
											<ul class="fh5co-sub-menu">
												<li>
													<a href="#" class="fh5co-sub-ddown">Movies</a>
													<ul class="fh5co-sub-menu">
														<li><a href="/memberhome/viewcontentmem">Bangla</a></li>
														<li><a href="/memberhome/hindiviewcontentmem" >Hindi</a></li>
														<li><a href="/memberhome/englishviewcontentmem">English</a></li>
														
													</ul>
												</li>
												
												<li>
													<a href="#" class="fh5co-sub-ddown">Tv Series</a>
													<ul class="fh5co-sub-menu">
														<li><a href="/memberhome/banglatvviewcontentmem">Bangla</a></li>
														<li><a href="#">Hindi</a></li>
														<li><a href="#" target="_blank">English</a></li>
														
														
													</ul>
												</li>
												
												
												<li>
													<a href="#" class="fh5co-sub-ddown">Games</a>
													<ul class="fh5co-sub-menu">
														<li><a href="#">0000</a></li>
														<li><a href="#">0000</a></li>
														
														
														
													</ul>
												</li>
												
												
												<li>
													<a href="#" class="fh5co-sub-ddown">Softwares</a>
													<ul class="fh5co-sub-menu">
														<li><a href="#">Electrical</a></li>
														<li><a href="#">Computer Science</a></li>
														
														
													</ul>
												</li>
												
											</ul>
										</li>
										
										<li><a href="#footer">Contact</a></li>
									</ul>
								</nav>
							</div>
						</div>
					</header>
				</div>
				<!-- end:fh5co-header -->
				<aside id="fh5co-hero" class="js-fullheight">
					<div class="flexslider js-fullheight">
						<ul class="slides">
							<li style="background-image: url({{asset('template2/')}}images/slide_1.jpg);">
								<div class="overlay-gradient"></div>
								<div class="container">
									<div class="col-md-5 col-sm-6 col-xs-12 js-fullheight slider-text">
										<div class="slider-text-inner">
											<div class="desc">
												<h2>Visit More...........</h2>
												
											</div>
										</div>
									</div>
								</div>
							</li>
							<li style="background-image: url({{asset('template2')}}/images/slide_2.jpg);">
								<div class="overlay-gradient"></div>
								<div class="container">
									<div class="col-md-5 col-sm-6 col-xs-12 js-fullheight slider-text">
										<div class="slider-text-inner">
											<div class="desc">
												<h2>Download More..............</h2>
												
											</div>
										</div>
									</div>
								</div>
							</li>
							<li style="background-image: url({{asset('template2')}}/images/slide_3.jpg);">
								<div class="overlay-gradient"></div>
								<div class="container">
									<div class="col-md-5 col-sm-6 col-xs-12 js-fullheight slider-text">
										<div class="slider-text-inner">
											<div class="desc">
												<h2>And Enjoy Your Life......</h2>
												
											</div>
										</div>
									</div>
								</div>
							</li>
						</ul>
					</div>
				</aside>
				
				<footer>
					<div id="footer">
						<div class="container">
							<div class="row">
								<div class="col-md-3">
									<h3 class="section-title">About Us</h3>
									<p>In this era of technology when most of our work depends on technology, our problems & their solutions also come out from it. So, it’s important to understand technology and the ways to use it properly in order to make life easy and comfortable, not the other way around. The reason we created Jvy is to guide people properly in solving their problems related to technology.</p>
								</div>
								
								<div class="col-md-33">
									<h3 class="section-title">Our Address</h3>
									<ul class="contact-info">
										<li><i class="icon-map2"></i>Nikunja-2, Khilkhet Dhaka 1229, Bangladesh</li>
										<li><i class="icon-phone2"></i>111111111111111</li>
										<li><i class="icon-envelope2"></i><a href="#">ftpserver@gmail.com</a></li>
										
									</ul>
								</div>
								
							</div>
							<div class="row copy-right">
								<div class="col-md-6 col-md-offset-3 text-center">
									<p class="fh5co-social-icons">
										<a href="#"><i class="icon-twitter2"></i></a>
										<a href="#"><i class="icon-facebook2"></i></a>
										<a href="#"><i class="icon-instagram"></i></a>
										<a href="#"><i class="icon-dribbble2"></i></a>
										<a href="#"><i class="icon-youtube"></i></a>
									</p>
									<p> All Rights Reserved.</p>
								</div>
							</div>
						</div>
					</footer>
					
				</div>
				<!-- END fh5co-page -->
			</div>
			<!-- END fh5co-wrapper -->
		</div>
		<!-- jQuery -->
		<script src="{{asset('template2')}}/js/jquery.min.js"></script>
		<!-- jQuery Easing -->
		<script src="{{asset('template2')}}/js/jquery.easing.1.3.js"></script>
		<!-- Bootstrap -->
		<script src="{{asset('template2')}}/js/bootstrap.min.js"></script>
		<!-- Waypoints -->
		<script src="{{asset('template2')}}/js/jquery.waypoints.min.js"></script>
		<!-- Superfish -->
		<script src="{{asset('template2')}}/js/hoverIntent.js"></script>
		<script src="{{asset('template2')}}/js/superfish.js"></script>
		<!-- Flexslider -->
		<script src="{{asset('template2')}}/js/jquery.flexslider-min.js"></script>
		<!-- Main JS (Do not remove) -->
		<script src="{{asset('template2')}}/js/main.js"></script>
	</body>
</html>